...
var (
	...
	// activationRegex matches an activation marker line (delta D6).
	activationRegex = regexp.MustCompile(`(?i)^\s*(?:activate|deactivate)\s+\S+$`)
...
func Parse(input string) (*SequenceDiagram, error) {
	...
	for i, line := range lines {
		...
		// Delta D6: activation markers. Upstream has no activation model, so
		// `activate B` / `deactivate B` used to fail the whole diagram. They
		// are tolerated (skipped) here so a diagram that uses activations
		// still renders — the activation bars themselves are not drawn, which
		// is noted in the vendored CHANGELOG as the obvious next delta.
		if activationRegex.MatchString(trimmed) {
			continue
		}

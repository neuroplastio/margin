...
// route on the first meaningful line (delta D2: the library's API surface is
// uniform across diagram kinds, mirroring sequence.SequenceDiagramKeyword).
const Keyword = "erDiagram"
...
func IsErDiagram(input string) bool {
	for _, line := range strings.Split(input, "\n") {
		...
		return low == strings.ToLower(Keyword) ||
			strings.HasPrefix(low, strings.ToLower(Keyword)+" ")
	}
	return false
}

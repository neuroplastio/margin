...
import (
	...
	log "github.com/neuroplastio/margin/internal/mermaid-ascii/pkg/log"
)

// textGraphData is an ordered string→[]textEdge map, the parse-stage data
// store. Upstream used github.com/elliotchance/orderedmap/v2 here (delta D8
// replaced it: the only operations the renderer needs are ordered Set, Get
// and first-seen-order iteration, which this type covers with a slice plus a
// map — no external dependency). Insertion order is the order nodes first
// appear in the document.
type textGraphData struct {
	order []string
	items map[string][]textEdge
}

func newTextGraphData() *textGraphData {
	return &textGraphData{items: make(map[string][]textEdge)}
}

// Get returns the edges stored under key, and whether the key is present.
func (d *textGraphData) Get(key string) ([]textEdge, bool) {
	v, ok := d.items[key]
	return v, ok
}

// Set stores value under key, keeping first-seen insertion order.
func (d *textGraphData) Set(key string, value []textEdge) {
...
func (d *textGraphData) Keys() []string {
	return d.order
}
...
func addNode(node textNode, data *textGraphData, nodeSpecs map[string]graphNodeSpec) {
...
func setData(parent textNode, edge textEdge, data *textGraphData, nodeSpecs map[string]graphNodeSpec) {
...
func mermaidFileToMap(mermaid, styleType string) (*graphProperties, error) {
	...
	for _, line := range lines {
		...
		for _, nodeName := range data.Keys() {

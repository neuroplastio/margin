...
func wrapTextInColor(text, c, styleType string) string {
	...
	// Delta D5: the CLI branch used to wrap text in gookit/color ANSI when
	// stdout looked like a terminal. margin styles diagrams itself (lipgloss)
	// and the graph layout's own colours would fight it, so CLI output is
	// plain text here. The dependency is gone with the branch.
	return text
}
